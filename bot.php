<?php
// Telegram Bot для 2FA Nevermore
// Загрузи на https://console.telebothost.com/
...
// TeleBotHost передаёт данные через POST
...
try {
    ...
    apiRequest('sendMessage', ['chat_id' => $chat_id, 'text' => '❌ Ошибка подключения к БД']);
    exit;
}
...
if ($text === '/start') {
    apiRequest('sendMessage', [
        'chat_id' => $chat_id,
        'text' => "👋 Привет!\n\n📝 Привяжи аккаунт: /link НИКНЕЙМ\n🔐 Получи код: /auth"
    ]);
}
elseif ($text === '/help') {
    apiRequest('sendMessage', [
        'chat_id' => $chat_id,
        'text' => "📋 Команды:\n/link НИКНЕЙМ - Привязать\n/auth - Код\n/unlink - Отвязать"
    ]);
}
elseif (strpos($text, '/link') === 0) {
    $username = trim(str_replace('/link', '', $text));
    if (empty($username)) {
        apiRequest('sendMessage', ['chat_id' => $chat_id, 'text' => "❌ Пример: /link MyNick"]);
    } else {
        ...
        if ($stmt->fetch()) {
            ...
            apiRequest('sendMessage', ['chat_id' => $chat_id, 'text' => "❌ Аккаунт не найден"]);
        }
    }
}
elseif ($text === '/auth') {
